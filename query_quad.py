...
from src import MetaData, load_model, ImplicitRepresentation
...
def render_sdf_quad(render_path, contour_path, gradient_path, model, P0, P1, P2, device, res=800, batch_size=1000, **kwargs):    
    ...
    if gradient_path is not None:
        dist_values,grad_values = IL.utils.forward_in_batches(
            model.neural_model, pts, device, 
            compute_grad=True, batch_size=batch_size)
    else:
        dist_values = IL.utils.forward_in_batches(model.neural_model, pts, device, compute_grad=False, batch_size=batch_size)
...
if __name__ == "__main__":
    ...
    IL.visualize.render_sdf_quad(render_path, contour_path, gradient_path, sdf, 
