...
model = IL.nn.DenseLipSDP(geometry.dim, 128, 10).to(DEVICE)
...
class CustomTrainer(Trainer):
    ...
    def forward_train_batch(self, data, model):
        Xin,Xout = data
        Xout = Xout + torch.randn_like(Xout)/1000.

        Yin = model(Xin)
        Yout = model(Xout)
        
        loss_attach = torch.sum((Yin+0.01)**2)
...
trainer.add_callbacks(
    ...
    callbacks.CheckpointCB(OUTPUT_DIR,[10*i for i in range(20)]),
