...
####### Dataset Sampling
n_points = 100_000 if geometry.geom_type is not IL.data.GeometryType.POINT_CLOUD_3D else len(geometry.vertices)
...
for i_iter in range(N_RESAMPLING):
    ...
    if i_iter !=0:
        ...
        while n_points_out<n_points:
            batch = IL.queries.sample_iso_raytraced(model, n_points, device=DEVICE, iso=MARGIN, threshold=1e-2)
        ...
        pc = M.mesh.from_arrays(points)
        pc.vertices.register_array_as_attribute("val", val)
        M.mesh.save(pc, os.path.join(OUTPUT_DIR, f"train_pts_{i_iter}.geogram_ascii"))

    trainer.set_training_data(train_data)
    
    trainer.add_callbacks(
        callbacks.LoggerCB(os.path.join(OUTPUT_DIR, "training_log.txt")),
        callbacks.MarchingCubeCB(OUTPUT_DIR, N_EPOCHS, res=200, iso=[-0.01, 0., 0.01])
    )

    model = IL.nn.DenseLipSDP(geometry.dim, 128, 12).to(DEVICE)
    trainer.train(model, starting_epoch=N_EPOCHS*i_iter)
