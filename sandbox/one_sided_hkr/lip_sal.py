...
####### Dataset Sampling
n_points = 5000 if geometry.dim == 2 else 10_000
...
# model = IL.nn.DenseLipSDP(geometry.dim, 256, 10, activation=nn.Softplus(10)).to(DEVICE)
model = IL.nn.DenseLipSDP(geometry.dim, 256, 10, activation=nn.ReLU()).to(DEVICE)
...
class CustomTrainer2D(Trainer):
    ...
    def forward_train_batch(self, data, model):
        ...
        # loss_out = torch.sum(-torch.abs(Yout))
    ...
    def get_optimizer(self, model):
        ...
        # return torch.optim.SGD(model.parameters(), lr=self.config.LEARNING_RATE, momentum=0.5)
...
if geometry.dim == 2 :
    IL.visualize.render_sdf_2d(None, os.path.join(OUTPUT_DIR, "contour_init.png"), os.path.join(OUTPUT_DIR, "gradient_init.png"), model, domain=M.geometry.AABB([-1.5,-1.5],[1.5,1.5]), device=DEVICE, res=800, batch_size=config2D.TEST_BATCH_SIZE)
elif geometry.dim == 3:
