...
"""
Projection of the outside points onto the 0 level set of the current iteration
"""
...
class CustomTrainer(Trainer):
    ...
    def forward_train_batch(self, data, model):
        ...
        loss_eq = torch.var(Yin) + torch.var(Yout)
...
def update_outside_distribution(points_on, model, margin, eta=0.5):
    dist = IL.utils.forward_in_batches(model, points_on, DEVICE)
    points_out =  IL.queries.project_onto_iso(points_on, model, iso=margin, device=DEVICE, normalize_grad=True)
    # points_out = IL.queries.sample_iso(points_on, model, iso=margin, device=DEVICE)
    points_out -= eta*np.clip(np.abs(dist)-margin, a_min=0, a_max=None)*( points_out-points_on)/np.linalg.norm(points_out-points_on, axis=1)[:,None]
    return points_out


margin = 0.05
lmbd = 100.
trainer = CustomTrainer(config, margin, lmbd)
trainer.add_callbacks(
    callbacks.LoggerCB(os.path.join(OUTPUT_DIR, "training_log.txt")),
)
if geometry.dim == 3:
    trainer.add_callbacks( callbacks.MarchingCubeCB(OUTPUT_DIR, config.N_EPOCHS, res=100, iso=[-0.3, -0.2, -0.1, 0.]) )
else:
    trainer.add_callbacks( callbacks.Render2DCB(OUTPUT_DIR, config.N_EPOCHS))

for i_iter in range(10):
    trainer.set_training_data(train_data)
    trainer.train(model, starting_epoch=config.N_EPOCHS*i_iter)

    # points_out = IL.queries.project_onto_iso(points_out, model, iso=margin, device=DEVICE, normalize_grad=True)
    points_out = update_outside_distribution(points_on, model, margin, 0.5)
    train_data = IL.data.make_tensor_dataset((points_on, points_out), DEVICE)    
