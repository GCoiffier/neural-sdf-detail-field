...
lip = estimate_local_Lipschitz_constant(NDF, points, stdv=np.abs(0.8*val), device=DEVICE, batch_size=10_000)

pc_init = M.mesh.from_arrays(points)
pc_init.vertices.register_array_as_attribute("ndf", val)
pc_init.vertices.register_array_as_attribute("lip", lip)
M.mesh.save(pc_init, os.path.join(OUTPUT_DIR, "init_points.geogram_ascii"))

MARGIN = np.max(np.abs(val))
print("Max error :", MARGIN)
if not os.path.exists(os.path.join(OUTPUT_DIR, "contour_init.png")):
...
# render_lip_constant_2d(os.path.join(OUTPUT_DIR,"local_lip_constant.png"), NDF, PLOT_DOMAIN, device=DEVICE, batch_size=10_000)
...
class CompactSupportPlateSpline(M.Worker):
    ...
    def __init__(self, points: np.ndarray, values: np.ndarray, lip: np.ndarray, **kwargs):
        super().__init__("CompactSplines", True)

        self.points : np.ndarray = np.asarray(points)
        self.values : np.ndarray = np.asarray(values)
        self.lip : np.ndarray = np.asarray(lip) * kwargs.get("alpha", 0.95)
        self.window = 4*np.amax(np.abs(self.values))
        self.weights = None
    ...
    # def rbf(self, r, L, V):
    #     return np.maximum(np.squeeze(np.abs(V)) - L*np.sign(r)*r, 0.)
    ...
    def _evaluate_rbf(self, x):
        near_x = self.tree.query_ball_point(x, self.window)
        if not near_x: return 0.
        dist_values = np.linalg.norm(self.points[near_x]-x, axis=1)
        lip_values = self.lip[near_x]
        values = self.values[near_x]
        return np.dot(self.weights[near_x], self.rbf(dist_values, lip_values, values))
    ...
    def run(self):
        ...
        for i in range(N):
            pi = self.points[i]
            near_i = self.tree.query_ball_point(self.points[i], self.window)
            for j in near_i:
                pj = self.points[j]
                phi_ij = self.rbf(M.geometry.distance(pi, pj), self.lip[i], self.values[i])
                vals.append(phi_ij)
                rows.append(i)
                cols.append(j)
...
rbf = CompactSupportPlateSpline(points, -val, lip)
rbf.run()


def render_detail_field(contour_path, details_path, model, details, domain : M.geometry.AABB, device, res=1000, batch_size=1000):
    assert domain.dim == 2
    ...
    print("min details:", np.amin(detail_values))
    print("max details:", np.amax(detail_values))
    ...
    if details_path is not None:
        ...
        plt.contour(img, levels=16, colors='k', linestyles="solid", linewidths=0.3)
        plt.contour(img, levels=[0.], colors='k', linestyles="solid", linewidths=0.6)
        plt.savefig(details_path, bbox_inches='tight', pad_inches=0, dpi=200)

render_detail_field(os.path.join(OUTPUT_DIR, "contour_final.png"), os.path.join(OUTPUT_DIR, "contour_details.png"), NDF, rbf, PLOT_DOMAIN, DEVICE)