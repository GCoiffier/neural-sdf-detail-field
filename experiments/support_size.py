...
support_size = np.max(distances_to_levelset)
...
for sigma in [0.1, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1., 2.]:
    ...
    mesh = M.transform.translate(mesh, M.Vec(-1, -1, -1)) # from [0,2]² to [-1,1]²
    ...
    M.mesh.save(mesh, os.path.join(args.folder, f"RBF_support_{sigma}.obj"))
    del support_values
    ...
    for v in mesh.id_vertices:
        ...
        vz = (1-dz)*L[2][iz] + dz * L[2][izn]
        mesh.vertices[v] = M.Vec(vx,vy,vz)
    M.mesh.save(mesh, os.path.join(args.folder, f"surface_with_details_{sigma}.obj"))
