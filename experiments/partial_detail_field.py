...
if __name__ == "__main__":
    ...
    res = 400
    sigma = 6.
    folder_path = os.path.join("output", "grayloc", "hkr")
    if not os.path.exists(folder_path):
        raise Exception("Grayloc folder not found. Please run `python train_hkr.py inputs/grayloc.obj` before running this script.")
    ...
    neural_model = load_model(metadata, folder_path, DEVICE, ignore_detail_field=True)
    ...
    allowed_volume = M.mesh.load(os.path.join("inputs/", "grayloc_partial_detail_allowed_volume.obj"))
    ...
    render_detail_field_3D(
        ...
        neural_model, rbf, plot_domain, DEVICE, res=res, batch_size=50_000, 
